...
class AnnouncementForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema->components([
            Tabs::make('Pengumuman')
                ->columnSpanFull()
                ->tabs([
                    Tab::make('Konten')
                        ->columns(2)
                        ->schema([
                            // type diset otomatis ke "announcement" oleh model Announcement.
                            Select::make('category_id')
                                ->label('Kategori')
                                ->options(fn () => Category::query()->where('type', 'post')->pluck('name', 'id'))
                                ->searchable()
                                ->preload(),
                            Toggle::make('is_featured')->label('Sematkan di atas (pinned)?'),
                            ...
                            Textarea::make('excerpt')->label('Ringkasan')->maxLength(500)->rows(2)->columnSpanFull()
                                ->helperText('Tampil di daftar pengumuman & meta deskripsi.'),
                            FileUpload::make('cover_path')->label('Gambar Sampul')->image()->directory('posts/covers')->maxSize(4096)->imageEditor()->columnSpanFull(),
                            RichEditor::make('body')->label('Isi Pengumuman')->required()->columnSpanFull()
